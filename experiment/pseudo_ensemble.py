...
def report_result(args, model, imgs, poses, hwf, bound):
    ...
    for img, rays_o, rays_d in zip(imgs, ray_origins, ray_directions):
        ...
        with torch.no_grad():
            ...
            # store for mae test
            pred_imgs.append(synth)
            pred_densities.append(denth)
            gt_imgs.append(img)
            # psnr
    ...
    return scene_psnr, scene_lpips, scene_ssim, pred_imgs, pred_densities, gt_imgs
...
def test():
    ...
    for idx, (imgs, poses, hwf, bound) in enumerate(test_loader):
        ...
        for epoch_id in range(outer_epoch):
            ...
            # scene_psnr, scene_lpips, scene_ssim, _,_,_ = report_result(args, final_model, test_imgs, test_poses, hwf, bound)
            ...
            pseudo_imgs_ensemble.clear()
            pseudo_density_ensemble.clear()           
            for i in range(len(temp_model_ensemble)):
                _,_,_, pred_imgs, pred_densitys, gt_imgs = report_result(args, temp_model_ensemble[i].to("cuda:0"), test_imgs, test_poses, hwf, bound)
                pseudo_imgs_ensemble.append(pred_imgs)
                pseudo_density_ensemble.append(pred_densitys)
            
            if epoch_id==0:
                pred_imgs, pred_uncertaintys = calculate_uncertainty()
                torch.save(pred_imgs, './mae_test/img_{}.pt'.format(idx+1))
                torch.save(pred_uncertaintys, './mae_test/uncertaintys_{}.pt'.format(idx+1))
                torch.save(gt_imgs, './mae_test/gt_{}.pt'.format(idx+1))
