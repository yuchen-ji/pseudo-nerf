...
def test_time_optimize_with_CL_pixels(args, model, optim, imgs, poses, hwf, bound, inner_steps):
    lamida1 = 0.8
    lamida2 = 0.15
    ...
    for step in range(inner_steps):
        ...
        # ========= here we include SPL to calculate v as loss weight =========
        # losses_pixels, weight will be calculated
        losses_pixels = torch.norm((colors - pixelbatch), p=2, dim=1, keepdim=True)
        num_gt_imgs = 4
        num_gt_pixels = len(torch.where(indices<num_gt_imgs*128*128)[0])

        # here we defined different SP-regularizers
        def Mixture(x, lamida1, lamida2, num_gt_pixels):
            x = x.detach().cpu().numpy()
            x = x / max(x)
            zeta = lamida1*lamida2 / (lamida1 - lamida2)
            weight = []
            for idx, item in enumerate(x):
                # if pixels in labeled image, we choose it
                if idx < num_gt_pixels:
                    weight.append([1, 1, 1])
                    continue
                if item < lamida2:
                    weight.append([1, 1, 1])
                    ...
                    temp = zeta * (1/item - 1/lamida1)
                    weight.append([temp, temp, temp])
            return weight

        weight = Mixture(losses_pixels, lamida1, lamida2, num_gt_pixels)
...
def test():
    ...
    savedir = Path("./videosCL_4v_4v_pixel")
    ...
    for idx, (imgs, poses, hwf, bound) in enumerate(test_loader):
        ...
        for epoch_id in range(pseudo_num + 1):
            ...
            test_time_optimize_with_CL_pixels(args, temp_model, optim, mixed_imgs, mixed_poses, hwf, bound, inner_steps[0])

            # if not the last epoch, then generate pseudo_imgs
            if epoch_id+1 != pseudo_num+1:
                pseudo_pose = get_delta_pose_all(source_poses, device, num_poses=(epoch_id+1), pose_range=((epoch_id+1)*math.pi/2.5))
        ...
        create_360_video(args, model, hwf, bound, device, idx+1, savedir, suffix='CL_4v_4v_pixel')
        
        print(f"scene {idx+1}, psnr:{scene_psnr:.3f}, video created")
        test_psnrs.append(scene_psnr)

        # write psnr to file
        with open('./videosCL_4v_4v_pixel/psnr.txt','a') as f:
