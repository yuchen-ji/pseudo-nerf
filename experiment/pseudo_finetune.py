...
from utils.shape_video import create_360_video, get_delta_pose
...
def test():
    ...
    for idx, (imgs, poses, hwf, bound) in enumerate(test_loader):
        ...
        mixed_imgs = source_imgs     # use these pseudo & labeled images to fine_tune!
        mixed_poses = source_poses  # use these pseudo & labeled poses to fine_tune!
        ...
        for epoch_id in range(pseudo_num + 1):
            ...
            test_time_optimize(args, temp_model, optim, mixed_imgs, mixed_poses, hwf, bound, inner_steps[0])

            # if not the last epoch, then generate pseudo_imgs
            if epoch_id+1 != pseudo_num+1:
                pseudo_pose = get_delta_pose(source_poses, device, (epoch_id+1)*math.pi/40, (epoch_id+1)*math.pi/40)
                # pseudo_pose = get_delta_pose(source_poses, device, 0, 0)
        ...
        # write psnr to file
        with open('./videos_4v_4v/psnr.txt','a') as f:
